...
**Superseded later the same day — `v0.8.0`.** The two paragraphs above
describe ADR-KEM-014 as accepted and unfinished, which was true of `v0.7.0`.
karyalay-mail then took the next move it was assigned: `0a06075` carded C.121
`listEffectiveRestrictions` and put "If-Match honoured when supplied" on
C.101/C.102, and `ac91510` implemented all three. `v0.8.0` regenerates from
that spec, so ADR-KEM-014 is now fully published. Delta AI-01 stays PARTIAL on
its one remaining defect, which has no ADR.
